...
//C source codes for the simulation program  --- The case is correponding to Fig.6a (empty circles and dotted lines) in the article
...
#define INOCUSTEP2 600000     // The step of the third inoculation

#define MECH_CHANGE_STEP 300000  // The step of changing relevant mechanism
...
#define PNPFR 0.3     // Probability of a nucleotide-precursor forming from its precursor catalyzed by NPR
...
#define FPP  30     // Factor of phospholipids on permeability (for nucleotides or their precursors)
...
int flag1, flag2, flag3, flagnr, flagnpr, flaggr, flagntsyn, flagnpsyn, flaggsyn; 
...
///////////////////////////////////////////////
void inoculate1(void)   // Inoculation of RNA into protocells -- version 1
{
	int k, k1, k2;
...
/////////////////////////////////////////////////////////////////////////
void inoculate2(void)   // Inoculation of RNA into protocells -- version 2
{
	int k, k1, k2;
	...
	for (y = 0; y < N; y++)   // Considering each room in the grid
	{
		...
		for (x = 0; x < N; x++)
		{
			...
			if (m_arr[y][x] > 0)  // Membrane existing (the room is occupied by a protocell)
			{
				...
				{
					if (findseq(nrseq, nrlength, p) == 0)flagnr = 0;
					else if (findseq(grseq, grlength, p) == 0)flaggr = 0;
				}
				if (flagnr == 0 && flaggr == 0)
				{
					k--;
					for (k1 = 0; k1 < INOCU_SEQ_NUM; k1++)
					{
						p2 = (struct rna*)malloc(LEN);
						if (!p2) { printf("\t%dform_monomer--memeout\n", k + 1); exit(0); }
						for (k2 = 0; k2 < inoculength5; k2++) p2->information[0][k2] = inocuseq5[k2];
						p2->information[0][k2] = 0;
						p2->information[1][0] = 0;
						p2->length1 = inoculength5;
						p2->length2 = 0;
						p2->next = room_head[0][y][x]->next;
						if (p2->next != room_head[0][y][x])(p2->next)->prior = p2;
						room_head[0][y][x]->next = p2;
	...
	if (k != 0) { printf("no sufficient cnrgr for inoculating npr"); }
}
...
void record(void)        // Data recording at every interval step (RECINT) 
{
	...
	printf("\n%d: nr=(%d)%d, gr=(%d)%d, contr1=(%d)%d, contr2=(%d)%d, npr=(%d)%d\n[tn=%d,rna=%d,np=%d,npp=%d,ta=%d,a=%d,tg=%d,g=%d,gp=%d,phl=%d(%d),c=%d,c0=%d,cnr=%d,cgr=%d,cnpr=%d,cnrgr=%d,cnrnpr=%d,cgrnpr=%d,cnrgrnpr=%d]\n",
		...
		(int)cell_num[g], (int)cell_0_num[g], (int)cell_nr_num[g], (int)cell_gr_num[g], (int)cell_npr_num[g], (int)cell_nrgr_num[g], (int)cell_nrnpr_num[g], (int)cell_grnpr_num[g], (int)cell_nrgrnpr_num[g]);
	...
	fprintf(fptxt1, "step=%d: nr=%d,  gr=%d, contr1=%d, contr2=%d, npr=%d, c=%d, c0=%d,cnr=%d,cgr=%d,cnpr=%d,cnrgr=%d,cnrnpr=%d,cgrnpr=%d,cnrgrnpr=%d, \n",
...
int main()
{
	...
	for (i = 0; i <= STEPNUM; i++)  // Monte-Carlo cycle
	{
		...
		if (i == INOCUSTEP1) inoculate1();  // Inoculation of NR,GR and Control-1 into empty protocells
		if (i == INOCUSTEP2) inoculate2();  // Inoculation of NPR into NR-GR protocells
